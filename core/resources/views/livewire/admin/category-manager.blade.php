<div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <h3 class="font-medium text-gray-900 mb-4">{{ $editingId ? 'Edit' : 'New' }} Category</h3>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <h3 class="font-medium text-gray-900 mb-4">All Categories</h3>
            @forelse ($categories as $category)
                <div class="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                    <div>
                        <span class="font-medium text-gray-900">{{ $category->name }}</span>
                        @if ($category->children->count())
                            <span class="text-xs text-gray-400 ml-1">({{ $category->children->count() }} children)</span>
                        @endif
                    </div>
                    <div class="flex items-center gap-2">
